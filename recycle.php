...
location.href='recycler.php?recycle='+a;
...
<table border=0 align=center width=70% height=70% id="table">
...
<tr align=left><td rowspan=3 width=15.6% align=left valign=top>
...
<?php
$i=0;
$sqllist="select count(slip) from recycle";
...
<font face=vani color=red><?php echo(date('d-m-y'));?><label style="margin-left:20px;">Recycle Medicine Bill</label></font>
...
$sql="select*from $t7 ORDER BY `slip` desc limit $l1,$l2";
$result=mysql_query($sql) or die("Sql100".mysql_error());
while($row=mysql_fetch_array($result))
{
?>
<tr><td><?php echo($row["slip"])?></td><td><?php echo $row["medicine"]?></td><td><?php echo $row["date"]?></td><td><?php echo $row["quantity"]?></td><td><?php echo $row["amount"]?></td><td style="border:none;"><a style="border:none;"href="javascript:recycle(<?php echo($row['slip']);?>)"><img src="photo/recycle.jpg" style="border:none;"></a></td></tr>
<?php
}
}
else
{
$sql="select*from $t7 ORDER BY `slip` desc limit 0,30;";
...
{
?>
<tr><td><?php echo($row["slip"])?></td><td><?php echo $row["medicine"]?></td><td><?php echo $row["date"]?></td><td><?php echo $row["quantity"]?></td><td><?php echo $row["amount"]?></td><td><a href="javascript:recycle(<?php echo($row['slip']);?>)"><img src="photo/recycle.jpg"></a></td></tr>

<?php
}
}
else{
echo("<tr><td valign=top colspan=6>No Medicine Deleted</td></tr>");
}
}
?>
</table>
